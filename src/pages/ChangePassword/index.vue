...
export default {
  data() {
    const validatePassword = (rule, value, callback) => {
      if (this.form.password_confirmation !== value)
        this.$refs.formChangePassword.validateField("password_confirmation");
      callback();
    };
    const validatePasswordConfirmation = (rule, value, callback) => {
      if (value === "")
        callback(new Error("O campo confirmar senha é obrigatório"));
      else if (value !== this.form.password)
        callback(
          new Error("O campo senha e confirmar senha devem ser iguais!")
        );
      callback();
    };
    return {
  ...
  async mounted() {
    await this.$nextTick(() => this.$refs.password.focus());
  },
  methods: {
    validate() {
